...
export function TransactionList({
  ...
  return <div className="space-y-6">
      ...
      const dayTotal = dayTransactions.reduce((sum, tx) => sum + (tx.type === 'expense' ? -tx.amount : tx.amount), 0);
      return <div key={date} className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-slate-50 font-bold text-sm">{month} {day}</h3>
      ...
      {showViewAll && transactions.length > 0 && <div className="text-center pt-2">
